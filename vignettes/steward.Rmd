...
The functions `stw_mutate_meta()` and `stw_mutate_dict()` work a little bit like `dplyr::mutate()`. However, instead of mutating the colunms of a data frame, `stw_mutate_meta()` mutates the top-level metadata, and `stw_mutate_dict()` mutates the *descriptions* of columns in a data frame:
...
If you are creating an R Markdown document, you can use the `stw_to_table()` function to create a [gt](https://gt.rstudio.com) table:

```{r}
stw_to_table(diamonds)
```
